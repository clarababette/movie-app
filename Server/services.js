...
function Services(db) {
  ...
  const login = async (req, res) => {
    ...
    await db.one('select password, id from users where username = $1', [username]).then(result => {
...
const handleRefreshToken = (req, res) => {
  ...
  const addToPlaylist = async (req, res) => {
    ...
    await db.none('insert into movies(movie_name, poster_url, user_id) select $1, $2, users.id from users where users.username = $3', [title, poster, username]).then(res.send('added')
    ).catch(err => res.send(err)) 
  };
  const removeFromPlaylist = async (req, res) => {
    const { username } = req.params;
    const { title, poster } = req.body;
    await db.none('delete from movies using users where user_id = users.id and users.username=$1 and movie_name = $2 and poster_url = $3',[username, title, poster]).then(res.send('removed')).catch(err => res.send(err))
  };
  return {
    signup,
    login,
